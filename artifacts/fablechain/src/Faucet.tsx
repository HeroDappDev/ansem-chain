...
export const Faucet: React.FC = () => {
  ...
  useEffect(() => {
    const savedWallet = localStorage.getItem('ansemchain_wallet');
    if (savedWallet) {
      try {
  ...
  const claimFaucet = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage({ type: 'success', text: `[+] Claimed ${data.amount} ANSEMCHAIN` });
        fetchWalletBalance(wallet.address);
        fetchFaucetStatus();
        fetchRecentTransactions();
      } else {
  ...
  const stakeTokens = async () => {
    ...
    if (amount < 10) {
      setMessage({ type: 'error', text: 'Minimum stake is 10 ANSEMCHAIN' });
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage({ type: 'success', text: `[+] Staked ${amount} ANSEMCHAIN` });
        setStakeAmount('');
        fetchWalletBalance(wallet.address);
        fetchStakingPosition();
        fetchPoolStats();
  ...
  const claimRewards = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage({ type: 'success', text: `[+] Claimed ${data.rewards.toFixed(4)} ANSEMCHAIN rewards` });
        fetchWalletBalance(wallet.address);
        fetchStakingPosition();
        fetchRecentTransactions();
      } else {
  ...
  const unstakeTokens = async (amount?: number) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage({ type: 'success', text: `[+] Unstaked ${data.unstaked} ANSEMCHAIN` });
        setUnstakeAmount('');
        fetchWalletBalance(wallet.address);
        fetchStakingPosition();
        fetchPoolStats();
  ...
  const compoundRewards = async () => {
    ...
    if (pendingRewards < 0.01) {
      setMessage({ type: 'error', text: 'Minimum compound amount is 0.01 ANSEMCHAIN' });
      return;
    }
    ...
    try {
      ...
      if (claimData.success && claimData.rewards >= 0.01) {
        ...
        if (stakeData.success) {
          setMessage({ type: 'success', text: `[+] Compounded ${claimData.rewards.toFixed(4)} ANSEMCHAIN` });
          fetchWalletBalance(wallet.address);
          fetchStakingPosition();
          fetchPoolStats();
          fetchRecentTransactions();
        } else {
          setMessage({ type: 'success', text: `[+] Claimed ${claimData.rewards.toFixed(4)} ANSEMCHAIN` });
        }
      } else {
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          {[
            { label: 'BALANCE', value: wallet.balance.toLocaleString(), suffix: 'ANSEMCHAIN' },
            { label: 'TOTAL STAKED', value: poolStats?.totalStaked?.toLocaleString() || '0', suffix: 'ANSEMCHAIN' },
            { label: 'APY', value: `${poolStats?.apy || 12}%`, suffix: '+ BONUS' },
            { label: 'STAKERS', value: poolStats?.totalStakers || 0, suffix: 'ACTIVE' },
            { label: 'REWARDS PAID', value: poolStats?.totalRewardsPaid?.toFixed(2) || '0', suffix: 'ANSEMCHAIN' },
          ].map((stat, i) => (
            <div key={i} style={{
              background: 'var(--cc-bg-secondary)',
              padding: '16px',
            ...
            <div style={{
              ...
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <div style={{ color: 'var(--cc-text-primary)', fontSize: '32px', fontWeight: 700, marginBottom: '4px' }}>
                  100 <span style={{ color: 'var(--cc-coral)', fontSize: '16px' }}>ANSEMCHAIN</span>
                </div>
                ...
                >
                  {isLoading ? 'CLAIMING...' : faucetStatus?.canClaim ? 'CLAIM ANSEMCHAIN' : `WAIT ${formatTimeUntil(faucetStatus?.nextClaimAt || 0)}`}
                </button>
              </div>
            </div>
              ...
              <div style={{
                ...
                {nextTier && (
                  <div style={{ marginBottom: '16px', fontSize: '10px', color: 'var(--cc-text-muted)', textAlign: 'center' }}>
                    {(nextTier.min - stakingPosition.amount).toLocaleString()} ANSEMCHAIN to {nextTier.name} (+{nextTier.bonus}% bonus)
                  </div>
                )}
              ...
              <div style={{ padding: '24px' }}>
                {activePanel === 'stake' && (
                  <>
                    ...
                    >
                      {isLoading ? 'STAKING...' : 'STAKE ANSEMCHAIN'}
                    </button>
                  </>
                )}
                    ...
                    {!stakingPosition ? (
                      ...
                      <>
                        <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                          ...
                          <div style={{ color: 'var(--cc-text-muted)', fontSize: '10px' }}>ANSEMCHAIN</div>
                        </div>
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <span style={{ color: 'var(--cc-text-muted)', fontSize: '10px' }}>
                          {tier.min.toLocaleString()}+ ANSEMCHAIN
                        </span>
                      </div>
