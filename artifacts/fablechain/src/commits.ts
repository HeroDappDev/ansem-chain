...
export const REPO_URL = 'https://github.com/HeroDappDev/ansem-chain';
const GITHUB_API = 'https://api.github.com/repos/HeroDappDev/ansem-chain/commits';
...
const AUTHORS = ['ANSEM-5', 'ansem-agent', 'ansem-worker'];
...
const MESSAGES = [
  ...
  'docs: document Ansem staking lifecycle',
...
export function generateCommits(count: number): GhCommit[] {
  const out: GhCommit[] = [];
  let t = Date.now();
...
export async function getCommits(count: number): Promise<GhCommit[]> {
  const real = await fetchRealCommits(count);
  return real ?? generateCommits(count);
}
