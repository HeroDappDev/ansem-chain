...
export const Wallet: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      const legacyKey = atob('YmxhY2tidWxsX3dhbGxldA==');
      const legacy = localStorage.getItem(legacyKey);
      if (legacy && !localStorage.getItem('ansemchain_wallet')) {
        localStorage.setItem('ansemchain_wallet', legacy);
      }
    ...
    if (savedWallet) {
      try {
        ...
        localStorage.removeItem('ansemchain_wallet');
      }
    }
  }, []);
  ...
  const fetchWallet = async (address: string) => {
    try {
      ...
      if (data.success && data.wallet) {
        ...
        setView('wallet');
        localStorage.setItem('ansemchain_wallet', JSON.stringify({ 
      ...
      const saved = localStorage.getItem('ansemchain_wallet');
      if (saved) {
        try {
  ...
  const createWallet = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem('ansemchain_wallet', JSON.stringify(data.wallet));
        setView('wallet');
  ...
  const importWallet = async () => {
    ...
    try {
      ...
      if (data.success) {
        setWallet(data.wallet);
        localStorage.setItem('ansemchain_wallet', JSON.stringify({ address: importAddress }));
        setView('wallet');
  ...
  const sendTokens = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMessage({ type: 'success', text: `[+] Sent ${data.amount} ANSEMCHAIN` });
    ...
    <div style={{ maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div style={{ color: 'var(--cc-text-primary)', fontSize: '36px', fontWeight: 700, marginBottom: '8px' }}>
          {wallet?.balance.toLocaleString()} <span style={{ color: 'var(--cc-coral)', fontSize: '18px' }}>ANSEMCHAIN</span>
        </div>
        <div style={{ 
          color: 'var(--cc-text-muted)', 
          fontSize: '11px', 
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ color: 'var(--cc-text-primary)', fontSize: '12px', fontWeight: 600 }}>
            {faucetStatus?.faucetAmount || 100} ANSEMCHAIN / 24h
          </div>
        </div>
